		...
		myvali:function(id){
			...
			var valiform={
				...
				vform:function(){
					...
					//勾选阅读同意
					// console.log(isCheckd.selector)
					if(isCheckd.selector){
						var formckd=myform.find(isCheckd).prop("checked");
						if(!formckd){
							myform.find(isCheckd).siblings("p").removeAttr("class").css({"color":errCol[0]}).html("请勾选阅读同意");
						}else{
							myform.find(isCheckd).siblings("p").attr({"class":"correct"}).css({"color":corrCol[0]}).html("√");
						}
					}
				}
			};
			...
			mybtn.on("click",function(){
				...
				if(cunt!=myform.find(myVali).length){
					...
					if(typeof mclass.callback=='function' && mclass.callback!=="undefined"){
						var forser=myform.serialize()
						mclass.callback(forser)
					}
				}
			})
		}
